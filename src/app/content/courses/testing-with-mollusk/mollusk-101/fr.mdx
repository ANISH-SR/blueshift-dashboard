...
// Compte payeur avec des lamports pour les transactions
let payer = Pubkey::new_unique();
let mut payer_account = Account::default();
payer_account.lamports = 100_000_000;
payer_account.owner = system_program::id();

// Compte non initialisé sans lamports
let uninitialized_account = Pubkey::new_unique();
let uninitialized_account_account = Account::default();
```
...
let mut program_account_account = Account::default();
program_account_account.lamports = lamports;
program_account_account.data = data;
program_account_account.owner = ID; // Le programme qui possède le compte
```

Pour les comptes avec des données, vous pouvez les initialiser comme ceci :
...
let mut program_account_account = Account::default();
program_account_account.lamports = lamports;
program_account_account.data = data.clone();
program_account_account.owner = ID;
```
...
> Lors de la création de comptes, vous pouvez utiliser `Account::default()` comme point de départ, puis définir les champs spécifiques dont vous avez besoin.
