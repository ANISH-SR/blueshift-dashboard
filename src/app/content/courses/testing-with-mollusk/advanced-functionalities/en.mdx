...
Mollusk's built-in token program helpers simplify testing scenarios involving SPL tokens. The `mollusk-svm` crate provides pre-configured support for `Token`, `Token2022`, and `Associated Token` programs out of the box.

When you create a new Mollusk instance, the essential token programs are already available for use in your tests. The built-in helpers will automatically handle the creation of token accounts, mints, and associated token accounts with proper initialization.
...
#[test]
fn test() {
    // All token programs are pre-loaded by default
    let mollusk = Mollusk::default();
    
    // Use the built-in token helpers for testing
    // No need to manually add token programs
}
```

For testing token operations, you can use the built-in token helpers. Here's an example of creating a new token account:

```rust
use solana_sdk::account::Account;
use solana_sdk::pubkey::Pubkey;

// Create a new token account with default values
let token_account = Account::default();

// Or create with specific data if needed
let mut custom_account = Account::default();
custom_account.lamports = 100; // Set initial lamports if needed
```
```
